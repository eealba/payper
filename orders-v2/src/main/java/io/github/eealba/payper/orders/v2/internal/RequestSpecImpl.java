...
abstract class RequestSpecImpl<T, T2, R1, R2> implements  RequestSpec<R1, R2>,
    ...
    public T withPayPalClientMetadataId(String paypalClientMetadataId) {
        requestBuilder.header("PayPal-Client-Metadata-Id", paypalClientMetadataId);
        return self();
    }

    @Override
    public T withPayPalPartnerAttributionId(String paypalPartnerAttributionId) {
        requestBuilder.header("PayPal-Partner-Attribution-Id", paypalPartnerAttributionId);
        return self();
    }
    @Override
    public T withPayPalAuthAssertion(String paypalAuthAssertion) {
        requestBuilder.header("PayPal-Auth-Assertion", paypalAuthAssertion);
        return self();
    }
    ...
    void pathParameter(String name, String value) {
